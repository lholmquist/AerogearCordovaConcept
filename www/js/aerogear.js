/*! AeroGear JavaScript Library - v1.0.0.M2 - 2013-01-03
...
(function( AeroGear, $, undefined ) {
    ...
    /*AeroGear.cordova = function( caller, options ) {
        var deferred = $.Deferred( function() {
            var that = this,
                settings = $.extend( {}, {
                    contentType: "application/json",
            ...
            var connectionSettings = $.extend( {}, settings, {
                success: function( data, textStatus, jqXHR ) {
                    that.resolve( typeof data === "string" && connectionSettings.dataType === "json" ? JSON.parse( data ) : data, textStatus, jqXHR );
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    if ( connectionSettings.dataType === "json" ) {
                        try {
                            jqXHR.responseJSON = JSON.parse( jqXHR.responseText );
                        } catch( error ) {}
                    }
            ...
            //make cordova call
            AeroGear.cordova.pipeline.read( connectionSettings );
        });
        var promise = deferred.promise();
